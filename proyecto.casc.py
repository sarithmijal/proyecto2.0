...
def obtener_ubicacion_actual():
    ...
    gmaps = googlemaps.Client(key='AIzaSyAO--KvovwVnSn-hiiEBAbp75lZxrnkGGM')
    geolocalizacion = gmaps.geolocate()
    latitud = geolocalizacion['location']['lat']
    longitud = geolocalizacion['location']['lng']
...
def sugerir_ventanas_no_utilizadas(tiempo_limite=600, ventanas_por_pagina=1):
    ...
    info_ventanas.append(f"Ubicación actual: Latitud: {latitud}, Longitud: {longitud}")
